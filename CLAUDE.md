...
- **Space + drag = pan** (up/down/sideways): while the pointer is over the 3D view, holding Space flips `controls.mouseButtons.LEFT` from ROTATE to PAN (`setPanMode`); Space is only intercepted over the viewport so it still toggles focused checkboxes elsewhere. Touch keeps OrbitControls' two-finger pan.
