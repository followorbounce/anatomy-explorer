...
- 1,629 named structures (1,635 in the source; 6 exact-duplicate meshes dropped by `drop_duplicates`; 6.68 M triangles source → 3.0 M after our decimation): skeleton 277, muscles 401, ligaments/fascia 31, heart 43, arteries 387, veins 216, brain 94, nerves 37, respiratory 26, digestive 47, urinary 6, genital 12, glands 4, eye/ear/face 39, skin 3, other 6.
...
- **Space + drag = pan** (up/down/sideways): while the pointer is over the 3D view, holding Space flips `controls.mouseButtons.LEFT` from ROTATE to PAN (`setPanMode`); Space is only intercepted while the pointer is over the viewport (tracked window-wide, so overlays don't break it), blurs any focused checkbox so key-up can't toggle it, and shows a "Pan mode" badge. Touch keeps OrbitControls' two-finger pan.
